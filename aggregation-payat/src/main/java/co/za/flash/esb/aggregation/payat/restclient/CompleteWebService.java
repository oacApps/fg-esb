...
@Service
public class CompleteWebService {
    ...
    public String backendService(HttpEntity<PayAtReqMdl> entity, String environment, LoggingPayloadMdl loggingPayloadMdl)  {
        ...
        CompleteResponseMdl responseMdl = new CompleteResponseMdl();
        ...
        try {
            ...
            if(null == responseEntity){
                ...
                LOGGER.error("CompleteWebService Response processing failed (101510): responseEntity is null");
            } else if(responseEntity.getStatusCodeValue() == 0){
                obBillPaymentResponse.setResponseCode(101510);
                obBillPaymentResponse.setResponseMessage("Response processing failed");
                responseMdl.setOBBillPaymentResponse(obBillPaymentResponse);
                responseEntity = ResponseEntity.status(HttpStatus.OK).body(responseMdl.toString());
                LOGGER.error("CompleteWebService Response processing failed (101510): responseEntity status code value is 0");
            }

        } catch (HttpStatusCodeException e) {
            LOGGER.error("CompleteWebService HttpStatusCodeException : " + e.getLocalizedMessage());
            ...
            LOGGER.error("CompleteWebService RestClientException : " + e.getLocalizedMessage());
            LOGGER.info("Response Entity : " + responseEntity.toString());
            e.printStackTrace();
            ...
            LOGGER.error("CompleteWebService RuntimeException : " + e.getLocalizedMessage());
            LOGGER.info("Response Entity : " + responseEntity.toString());
            e.printStackTrace();
            obBillPaymentResponse.setResponseCode(101500);
            obBillPaymentResponse.setResponseMessage("Internal Server error.");
        ...
        if (null == responseEntity.getBody()){
            ...
            responseMdl.setOBBillPaymentResponse(obBillPaymentResponse);
            responseEntity = ResponseEntity.status(HttpStatus.OK).body(responseMdl.toString());
            LOGGER.error("CompleteWebService Response processing failed (101510): responseEntity NULL");
        }
