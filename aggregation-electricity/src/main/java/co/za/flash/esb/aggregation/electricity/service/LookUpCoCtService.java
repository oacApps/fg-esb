...
import co.za.flash.esb.aggregation.electricity.mapper.LookUpCoCtRequestMapper;
...
import co.za.flash.esb.aggregation.electricity.restclient.LookUpCoCtWebService;
...
@Service
public class LookUpCoCtService {
    ...
    @Autowired
    LookUpCoCtWebService webService;

    public Object service(Map<String, String> headers, LookupCoCtRequestDTO lookupCoCtRequestDTO, boolean isTest) {

        LoggingPayloadMdl loggingPayloadMdl = new LoggingPayloadMdl();
        loggingPayloadMdl.setApiKey("electricity-lookup-COCT");
        loggingPayloadMdl.setRequestInPayload(lookupCoCtRequestDTO);
        ...
        try {
            ...
            if (headers.containsKey(wso2JwtKeys.xJwtAssertion)) {
                try {
                    ...
                    e.printStackTrace();
                    LOGGER.error("LookUp COCT Service jwtExtractor JWTException: " + e.getLocalizedMessage());
                } catch (Exception e) {
                    e.printStackTrace();
                    LOGGER.error("LookUp COCT Service wso2JwtKeys: " + e.getLocalizedMessage());
                }

                HttpHeaders httpHeaders = buildHeader.build(headers);
                // rest of the code
                ElectricityRequestMdl electricityRequestMdl = LookUpCoCtRequestMapper.INSTANCE.toMdl(lookupCoCtRequestDTO);
                ...
                try{
                    Gson gson = new Gson();
                    responseMdl = gson.fromJson(bkResponse, LookUpCoCTResponseMdl.class);

                    if(null == responseMdl.getOBElecResponse()){
                        OneBalance oneBalance = gson.fromJson(bkResponse, OneBalance.class);
                        /*OBElecLookupResponse obElecResponse = OneBalanceResponseMapper.INSTANCE.toOBElecLookupResponse(oneBalance.getOneBalance());
            ...
            e.printStackTrace();
            LOGGER.error("LookUp COCT Service : " + e.getLocalizedMessage());
            return new ResponseEntity("An internal server error occurred. Please try again later.", HttpStatus.INTERNAL_SERVER_ERROR);
        } finally {
            LOGGER.info("Payload: " + JsonStringMapper.toJsonString(loggingPayloadMdl));
        }
