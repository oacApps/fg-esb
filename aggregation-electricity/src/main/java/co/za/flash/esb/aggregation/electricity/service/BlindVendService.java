...
import co.za.flash.esb.aggregation.electricity.mapper.BlindVendRequestMapper;
...
import co.za.flash.esb.aggregation.electricity.model.response.BlindVendResponseMdl;
...
public class BlindVendService {
    ...
    public Object service(Map<String, String> headers, BlindVendRequestDTO blindVendRequestDTO, boolean isTest) {

        LoggingPayloadMdl loggingPayloadMdl = new LoggingPayloadMdl();
        loggingPayloadMdl.setApiKey("blindVend-lookup");
        loggingPayloadMdl.setRequestInPayload(blindVendRequestDTO);
        ...
        BlindVendResponseMdl responseMdl = new BlindVendResponseMdl();
        ...
        try {
            ...
            if (headers.containsKey(wso2JwtKeys.xJwtAssertion)) {
                ...
                // rest of the code
                ElectricityRequestMdl electricityRequestMdl = BlindVendRequestMapper.INSTANCE.toMdl(blindVendRequestDTO);
                ...
                try{
                    Gson gson = new Gson();
                    responseMdl = gson.fromJson(bkResponse, BlindVendResponseMdl.class);

                    if(null == responseMdl.getOBElecResponse()){
                        OneBalance oneBalance = gson.fromJson(bkResponse, OneBalance.class);
                       // OBElecBlindVendResponse obElecResponse = OneBalanceResponseMapper.INSTANCE.toOBElecBlindVendResponse(oneBalance.getOneBalance());
